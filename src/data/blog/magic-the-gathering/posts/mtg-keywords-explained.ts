...
- **Double strike.** The creature deals combat damage in *both* the first-strike step and the regular step — it effectively hits twice. A 3/3 with double strike deals 3, then another 3, for six total, and kills a first-strike blocker before taking any damage.
...
- **Indestructible.** An indestructible creature is not destroyed by lethal damage or by effects that say "destroy." Combat damage still gets marked on it, but it does not die from it, and a board wipe that says "destroy all creatures" leaves it standing. It is *not* immune to everything, though — it still dies to sacrifice effects, to -X/-X effects that drop its toughness to 0, and to exile (exile doesn't "destroy"). The reliable way to kill an indestructible creature is to exile it or shrink it to zero toughness.
