...
export const fortniteMatsManagementGuide: BlogPost = {
  ...
  content: `Running out of mats in the final circle isn't bad luck. It's a decision you made three circles earlier when you spam-built a tower over a fight that didn't matter, or never bothered to farm in the first place, or hoarded so hard you hit the cap and wasted every swing after. Material management is the quiet skill that decides Build-mode endgames, and almost nobody trains it on purpose — they just notice they're out of wood when it's already too late to fix.

Mats are a resource you spend to win fights and survive rotations, governed by a hard cap and a real trade-off between the three types. Manage them well and you arrive at the last high-ground battle with a full bank and the ability to keep re-walling while your opponent runs dry. Manage them badly and you're the player in the open with 40 wood, watching someone box you up. This guide is the whole resource picture — farming, the cap, the wood/brick/metal trade-offs, when to build versus save, and the heals and ammo that round out your inventory.
